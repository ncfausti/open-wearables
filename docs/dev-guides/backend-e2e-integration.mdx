...
description: "Complete walkthrough for integrating Oriya with your backend application"
---

This guide walks you through the complete integration flow for connecting your backend application to Oriya. You'll learn how to create users, connect wearable providers via OAuth, sync data, and retrieve health metrics.

## Prerequisites

Before you begin, ensure you have:

- Oriya instance running (self-hosted or cloud)
- API Key generated from the settings tab in Oriya Developer portal
...
<Tabs>
  <Tab title="When to Use">
    ...
    | **SDK Token** | Mobile apps pushing health data to Oriya |
  </Tab>
...
When a user registers in your application, create a corresponding user in Oriya.
...
<Warning>
**Store the Oriya User ID!** You'll need this ID for all subsequent API calls.
</Warning>

### Update Your User Model

Add a field to store the Oriya user ID:
...
Oriya allows multiple users with the same email. To prevent duplicates in concurrent requests (common in SPAs), implement a check-then-create pattern:
...
<Steps>
  ...
  <Step title="Your backend calls the authorize endpoint">
    Request the authorization URL from Oriya.
  </Step>
  <Step title="Redirect user to authorization_url">
    The user authenticates with their wearable provider.
  </Step>
  <Step title="Provider redirects to Oriya callback">
    Oriya handles the OAuth callback automatically.
  </Step>
  <Step title="Oriya redirects to your app">
...
Here's a complete Python client class for Oriya integration:
...
<AccordionGroup>
  <Accordion title="Docker container can't reach Oriya on localhost">
    If your app runs in Docker and Oriya runs on the host machine:
  ...
  <Accordion title="Multiple users created for the same email">
    Oriya allows duplicate emails by design. Use `external_user_id` to identify users uniquely:
