...
<Warning>
**Before you start:** The Strava integration is intended solely for the Oriya single-user usage scenario (i.e., personal use, not as an aggregator of multiple users’ data). Keep in mind:
...
<Note>
**Need help with your Strava integration?** Pop into our [Discord](https://discord.gg/qrcfFnNE6H) if you have questions or want to discover how Oriya can solve your problems.
</Note>
...
```
    ...
    When Strava receives this request, it will send a GET request to your `callback_url` with a `hub.verify_token` parameter. Oriya will validate the token against your `STRAVA_WEBHOOK_VERIFY_TOKEN` and echo back the challenge to confirm the subscription.
...
```
  ...
  <Step title="Verify the integration">
    ...
    1. Start your Oriya instance
    2. Connect a Strava account through the OAuth flow
    3. Create or record an activity on Strava
    4. The webhook should deliver the event and Oriya will fetch and store the full activity data
