...
<Note>
**Need help with your Ultrahuman integration?** Pop into our [Discord](https://discord.gg/qrcfFnNE6H) if you have questions or want to discover how Oriya can solve your problems.
</Note>
...
| Method | Description |
|--------|-------------|
| **Polling (pull)** | Oriya periodically fetches daily metrics via the Ultrahuman Partnership API (default: every hour via Celery Beat) |
...
```
  ...
  <Step title="Connect a user via OAuth">
    With credentials configured and your Oriya instance running, initiate the OAuth flow.
    ...
    **3. Ultrahuman redirects back** to the callback URI configured in your `.env`. Oriya automatically exchanges the authorization code for access and refresh tokens.
