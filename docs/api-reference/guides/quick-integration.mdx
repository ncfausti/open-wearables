...
description: 'End-to-end guide to integrate Oriya with your application'
---

## Overview

This guide walks you through the complete flow of integrating Oriya with your application, from creating a user to fetching their health data.

```
1. Create a user in Oriya
...
First, create a user in Oriya to represent your application's user.
...
<Tip>
  Use `external_user_id` to map Oriya users to your application's user IDs for easy lookup later.
</Tip>
...
<Warning>
  Oriya allows duplicate email addresses. If you need uniqueness, check for existing users first using `GET /api/v1/users?search=user@example.com` which returns a paginated list.
</Warning>
