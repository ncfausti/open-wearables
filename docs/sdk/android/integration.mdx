...
This guide walks you through the complete integration of the Oriya Android SDK into a native Kotlin application, from backend setup to production deployment.
...
<Steps>
  ...
  <Step title="SDK stores & syncs" icon="lock">
    Android SDK stores credentials in EncryptedSharedPreferences and uses `accessToken` to sync health data directly to Oriya.
  </Step>
</Steps>
...
|-----------|------|-------------|
| `host` | `String` | **Required.** The Oriya API base URL (host only, without path suffix) |
