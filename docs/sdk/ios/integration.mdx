...
This guide walks you through the complete integration of the Oriya iOS SDK into a native Swift application, from backend setup to production deployment.
...
<Steps>
  ...
  <Step title="SDK stores & syncs" icon="lock">
    iOS SDK stores credentials in Keychain and uses `accessToken` to sync health data directly to Oriya.
  </Step>
</Steps>
...
|-----------|-------------|
| `host` | The Oriya API base URL — host only, without path suffix (e.g. `https://api.openwearables.io`) |
