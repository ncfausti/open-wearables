...
This guide walks you through the complete integration of the Oriya Flutter SDK, from backend setup to production deployment.
...
<Steps>
  <Step title="Your Backend generates token" icon="server">
    Your backend calls the Oriya API with your **App credentials** (`app_id` + `app_secret`) to generate a user-scoped token (server-to-server, HTTPS) via [Create User Token](/api-reference/mobile-sdk/create-user-token) endpoint. Oriya returns `access_token` + `refresh_token`.
  </Step>
  ...
  <Step title="SDK stores & syncs" icon="lock">
    Flutter SDK stores credentials in iOS Keychain / Android EncryptedSharedPreferences and uses `accessToken` to sync health data directly to Oriya.
  </Step>
</Steps>
...
graph TD
    B["Your Backend"] -->|"&nbsp; POST /api/v1/users/{user_id}/token &nbsp;&nbsp; (app_id and app_secret) &nbsp;"| OW["Oriya API"]
...
Your backend needs a single endpoint that generates access tokens for your users by calling the Oriya API and forwarding the tokens.
...
2. Call Oriya API at [`POST /api/v1/users/{user_id}/token`](/api-reference/mobile-sdk/create-user-token) with your App credentials
...
app.post('/api/health/connect', authenticateUser, async (req, res) => {
  try {
    ...
    // Call Oriya API to generate a user-scoped token
...
<Note>
  The `user_id` in the URL is the Oriya User ID (UUID). You should store this mapping in your database when you first [Create User](/api-reference/users/create-user) via the Oriya API.
</Note>
...
|-----------|------|-------------|
| `host` | `String` | **Required.** The Oriya API base URL — host only, without path suffix (e.g. `https://api.openwearables.io`) |
...
<Info>
  The `userId` parameter is the **Oriya User ID** (UUID) — the `id` returned by the [Create User](/api-reference/users/create-user) endpoint. Do **not** pass your own `external_user_id` here.
</Info>
...
class HealthSyncService {
  ...
  Future<void> _signIn() async {
    // Call YOUR backend — not the Oriya API directly
...
<ParamField path="host" type="string" required>
  Your Oriya API base URL (e.g. `https://api.example.com`).
</ParamField>
...
Data is automatically normalized to the Oriya unified data model and can be accessed through the standard API endpoints.
