...
This guide walks you through the complete integration of the Oriya React Native SDK, from backend setup to production deployment.
...
<Steps>
  ...
  <Step title="SDK stores & syncs" icon="lock">
    React Native SDK stores credentials in iOS Keychain / Android EncryptedSharedPreferences and uses `accessToken` to sync health data directly to Oriya.
  </Step>
</Steps>
...
app.post('/api/health/connect', authenticateUser, async (req, res) => {
  try {
    ...
    // 2. Call Oriya API to generate token
...
|-----------|------|-------------|
| `host` | `String` | **Required.** The Oriya API URL (e.g. `https://api.openwearables.io`) |
